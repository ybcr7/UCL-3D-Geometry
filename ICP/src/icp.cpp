...
Eigen::MatrixXd ICP::ICPBasic(Eigen::MatrixXd V_target, Eigen::MatrixXd V_to_process){
    ...
    std::pair<Eigen::Matrix3d, Eigen::RowVector3d> transform = EstimateRigidTransform(V_matched, V_to_process);
    ...
    for (int x = 0; x < 3; x++){
        for (int y = 0; y < 3; y++){
            for (int z = 0; z < 3; z ++){
                ...
                Eigen::MatrixXd V_matched = FindCorrespondences(V_target, V_rotated);
